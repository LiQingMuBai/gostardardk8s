...
import (
	...
	"gostardardk8s/pkg/global/response"
	"gostardardk8s/pkg/middleware"
	"gostardardk8s/pkg/model"
	"gostardardk8s/pkg/model/request"
	resp "gostardardk8s/pkg/model/response"
	"gostardardk8s/pkg/service"
	utils "gostardardk8s/tools"
...
func Login(c *gin.Context) {
	...
	UserVerify := utils.Rules{
		//"CaptchaId": {tools.NotEmpty()},
		//"Captcha":   {tools.NotEmpty()},
		"Username":  {utils.NotEmpty()},
		"Password":  {utils.NotEmpty()},
	}
...
func tokenNext(c *gin.Context, user model.SysUser) {
	...
	if error :=service.SetRedisJWTTokenKey(token,user.Username); error != nil {
